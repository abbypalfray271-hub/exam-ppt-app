...
export async function chatWithGemini(
  ...
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 90_000); // 90s 超时，比 Cloudflare 524 更早报错
    ...
    if (error?.name === 'AbortError') {
      throw new Error('AI 请求超时（超过 90 秒），请缩小框选区域后重试');
    }
...
1. **逐题拆分（最重要！）**：
   ...
   - **100% 还原排版与简答作答区 (极度重要)**：如果原图中题目下方有供学生手写主观题的连续长下划线 \`________\` 或多行答题留白，你**必须**在 \`content\` 中对应位置输出足够长的下划线，忠实复刻原图的空白行与排版结构。你的输出就是一张真实的“答题卡”，绝对不能因为它们是单纯的长线而精简或省略！
...
4. **挖空填词与答案回填原则 (Cloze & Answer) —— 【核心指令】**：
   ...
   - 对于非填空题的大段解析/答案，请将其放入 \`analysis\` 字段，严禁放入 \`content\` 中。相关元数据（如难度、知识点、导语、详解）也请一并整理放入 \`analysis\`。
...
1. **逐题拆分 (一题一号)**：
   ...
   - **排版与简答卡格式 100% 还原**：图片内容中（特别是正文下方）如果要学生作答的区域存在长下划线或多行手写空白线，你必须在 \`content\` 的相应位置用重复的长下划线（如 \`________\`）一字不差地复刻并保留其换行结构。绝对禁止因为它们“没有文字含义”而被过滤掉！
...
4. **挖空填词与答案回填原则 (Cloze & Answer) —— 【核心指令】**：
   ...
   - 例如：输出 \`① {{甲骨文}} ② {{小篆}}\`。
   - 若为大段选择题解析，分析、详解、难度等信息请放入 \`analysis\` 字段，不要放入 \`content\` 中。
   - **视觉打码专用 (answer_box)**：
