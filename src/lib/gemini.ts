...
4. **答题位置精准回填 (Answer Positioning) —— 【核心优先级目标】**：
   ...
   - **原地回填原则**：严禁仅在底部显示答案而不回填位置！所有答案第一优先级是填入 \`content\` 对应的 \`{{}}\` 位置。
   - **解析分离原则**：【解析】、【详解】等内容必须独立追加在 \`content\` 底部。
   - **视觉打码专用 (answer_box)**：
     - 必须识别原图中印好的答案区坐标，用于前端遮罩。
...
7. **符号保真与清理原则 (Symbol Fidelity & Cleanup)**：
    - **优先 Unicode**：直接使用图片原样的数学符号（如 △, ∠, ⊥, //, °, ², ³），绝对禁止使用 LaTeX 语法转换此类简单符号（如严禁输出 \triangle, \angle）。
    - **严禁碎片化包裹**：禁止将普通英文字母或单位（如 t, S, cm, s, m/s）包裹在 $ 符号中。
    - **保持纯净文本**：除非是复杂的分段公式、分式、根式或极其复杂的数学表达式，否则一律使用纯文本。
...
4. **答题位置精准回填与智能补全 (Positioning & Supplement) —— 【核心优先级目标】**：
   ...
   - **视觉打码专用 (answer_box)**：
     - 若原图有印好的答案区，请输出坐标。若无（即 AI 自主生成），则 \`answer_box\` 可留空或输出题干末尾区域。
